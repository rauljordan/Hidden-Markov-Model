...
class HmmTest( unittest.TestCase ):
    ...
    def setUp(self):
        ...
        self.hmm = hmm.Hmm( self.init_seq, self.a, self.b )
    ...
    def test_roulette( self ):
        ...
        for x in xrange(1000):
            result = self.hmm.roulette( self.init_seq )
            stats[result] += 1
    ...
    def test_main_function( self ):
        seq_length = 10
        result_seq = self.hmm.generate( seq_length )
        self.assertTrue( len(result_seq) == seq_length )

    def test_viterbi_function( self ):
        obs = self.hmm.generate( 10 )
        obs = [6, 6]
        prob, sys_states = self.hmm.viterbi(obs)
